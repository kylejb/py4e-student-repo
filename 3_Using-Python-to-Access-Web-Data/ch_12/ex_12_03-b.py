...
## Following Links in Python | Alternative Approach
...
icount = int(count)
## QUESTION: Why is my output inaccurate without "minus 1"?
iposition = int(position) - 1
...
while icount > 0:
    ...
    for targetTag in targetTags:
        links.append(targetTag.get("href", None))
    # print(links)
    print("Retrieving:", links[iposition])
    html = urllib.request.urlopen(links[iposition], context=ctx).read()
    icount -= 1
